...
?>
        ...
        <?php
                   $query_edit = mysqli_query($connections, "SELECT * FROM logs WHERE 1;");
                   while($row_edit = mysqli_fetch_assoc($query_edit)){   
                        $log_time = $row_edit['time'];
                        $log_action = $row_edit['action'];
                        $log_user = $row_edit['user'];
                   }
         ?>
        <div class="side-nav">
        ...
        $view_query = mysqli_query($connections, "SELECT * FROM logs ORDER BY id DESC");
